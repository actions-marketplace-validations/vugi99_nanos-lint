...
- Glob wildcards now apply to `workspace.ignoreDir` entries as well (`deep/*` previously matched nothing, and an absolute path inside the workspace now excludes that directory), and unusable pattern values (`""`, `"."`, NUL bytes, oversized strings, non-string JSON values) are skipped instead of throwing out of `countCheckedFiles()` (#27).
